...
MODULE_DESCRIPTION(
	"RealTek RTL-8169 Gigabit Ethernet driver - IRQ Handler Optimized for Low Latency");
...
static irqreturn_t rtl8169_interrupt(int irq, void *dev_instance)
{
	 ...
	 * - Inlined status read for RTL8168 (most common)
	 * - Removed unlikely() hints that hurt branch prediction
	 * - Streamlined hot path to NAPI scheduling
	 */
	status = RTL_R16(tp, IntrStatus);

	/* Fast path: check if this is our interrupt */
	if (unlikely((status & 0xffff) == 0xffff || !(status & tp->irq_mask)))
		return IRQ_NONE;

	/*
	 * Hot path optimization: Schedule NAPI first for RX/TX.
	 * Rare events (SYSErr, LinkChg) handled after.
	 */
	if (napi_schedule_prep(&tp->napi)) {
		/* Inline IRQ disable for RTL8168 - avoid function call */
		RTL_W16(tp, IntrMask, 0);
		__napi_schedule(&tp->napi);
	}

	/* Rare event handling - cold path */
	if (status & SYSErr) {
