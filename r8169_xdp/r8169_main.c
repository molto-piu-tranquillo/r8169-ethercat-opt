...
struct rtl8169_private {
	...
	unsigned xdp_enabled : 1; /* XDP program attached */
	...
	/* XDP program and RX queue info */
	struct bpf_prog *xdp_prog;
	struct xdp_rxq_info xdp_rxq;

	/* AF_XDP zero-copy */
	struct xsk_buff_pool *xsk_pool;
	struct xdp_buff *rx_xdp_buff[NUM_RX_DESC]; /* XSK buffer tracking */
};
...
/* ========== Page Pool and XDP Support ========== */

static void rtl8169_xdp_mark_to_asic(struct RxDesc *desc, u32 buf_size)
{
	...
	dma_wmb();
	WRITE_ONCE(desc->opts1, cpu_to_le32(DescOwn | eor | buf_size));
}

static int rtl8169_setup_page_pool(struct rtl8169_private *tp)
{
	struct page_pool_params pp_params = {
		.order = 0,
		.flags = PP_FLAG_DMA_MAP | PP_FLAG_DMA_SYNC_DEV,
		.pool_size = NUM_RX_DESC * 2,
		.nid = dev_to_node(tp_to_dev(tp)),
		.dev = tp_to_dev(tp),
		.dma_dir = DMA_FROM_DEVICE,
		.offset = RTL_XDP_HEADROOM,
		.max_len = RTL_XDP_RX_BUF_SIZE - RTL_XDP_HEADROOM,
	};

	tp->page_pool = page_pool_create(&pp_params);
	if (IS_ERR(tp->page_pool)) {
		int err = PTR_ERR(tp->page_pool);
		tp->page_pool = NULL;
		return err;
	}
...
/* Allocate RX buffer from page_pool with XDP headroom */
static struct page *rtl8169_alloc_rx_data_xdp(struct rtl8169_private *tp,
					      struct RxDesc *desc)
{
	struct page *page;
	dma_addr_t dma;
	...
	/* page_pool provides DMA address with offset already applied */
	dma = page_pool_get_dma_addr(page) + RTL_XDP_HEADROOM;

	desc->addr = cpu_to_le64(dma);
	rtl8169_xdp_mark_to_asic(desc, RTL_XDP_RX_BUF_SIZE - RTL_XDP_HEADROOM);
		  ...
		  int budget)
{
	...
	for (count = 0; count < budget; count++, tp->cur_rx++) {
		...
		/* page_pool mode: data starts at headroom offset */
		if (tp->page_pool)
			rx_buf = page_address(page) + RTL_XDP_HEADROOM;
		else
			rx_buf = page_address(page);
		...
		if (xdp_prog) {
			...
			xdp_init_buff(&xdp, PAGE_SIZE, &tp->xdp_rxq);
			xdp_prepare_buff(&xdp, page_address(page),
					 RTL_XDP_HEADROOM, pkt_size, false);
			...
			default:
				...
				fallthrough;
			case XDP_ABORTED:
				/* trace_xdp_exception not available in out-of-tree module */
				fallthrough;
			case XDP_DROP:
				dev->stats.rx_dropped++;
				goto release_descriptor;
		...
		/* Normal skb path (XDP_PASS or no XDP) */
		if (tp->page_pool) {
			/* page_pool mode: build skb from page (zero-copy for stack) */
			skb = napi_build_skb(page_address(page), PAGE_SIZE);
			if (unlikely(!skb)) {
				dev->stats.rx_dropped++;
				goto release_descriptor;
			}
			skb_reserve(skb, RTL_XDP_HEADROOM);
			skb_put(skb, pkt_size);
			skb_mark_for_recycle(skb);
			page_reuse = false; /* skb now owns the page */
		} else {
			/* Legacy mode: allocate new skb and copy data */
...
next_desc:
		if (!page_reuse) {
			...
			if (tp->page_pool) {
				new_page = rtl8169_alloc_rx_data_xdp(tp, desc);
			} else {
				new_page = rtl8169_alloc_rx_data(tp, desc);
			}
...
static int rtl8169_poll(struct napi_struct *napi, int budget)
{
	...
	/* Use zero-copy RX path if AF_XDP is active */
	if (tp->xsk_pool)
...
static int rtl_open(struct net_device *dev)
{
	...
	/* Setup page_pool for XDP mode (always enabled for now) */
	retval = rtl8169_setup_page_pool(tp);
	if (retval < 0)
		goto err_free_rx_1;
...
static int rtl8169_xsk_pool_setup(struct net_device *dev,
				  struct xsk_buff_pool *pool, u16 qid)
{
	...
	if (qid >= 1) {
		netdev_err(dev, "Invalid queue id %d for AF_XDP\n", qid);
		return -EINVAL;
	}
...
static int rtl8169_xdp(struct net_device *dev, struct netdev_bpf *xdp)
{
	switch (xdp->command) {
	case XDP_SETUP_PROG:
		return rtl8169_xdp_setup(dev, xdp->prog, xdp->extack);
	case XDP_SETUP_XSK_POOL:
		return rtl8169_xsk_pool_setup(dev, xdp->xsk.pool,
					      xdp->xsk.queue_id);
	default:
		return -EINVAL;
	}
