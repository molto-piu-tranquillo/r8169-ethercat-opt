...
/* Zero-copy RX optimization constants */
#define RTL_SKB_PAD (NET_SKB_PAD + NET_IP_ALIGN)
#define RTL_RX_PAGE_SIZE 2048
#define RTL_RX_TRUESIZE                                  \
	SKB_DATA_ALIGN(RTL_SKB_PAD + RTL_RX_PAGE_SIZE) + \
...
MODULE_DESCRIPTION(
	"RealTek RTL-8169 Gigabit Ethernet driver - Zero-copy RX Optimized for Low Latency");
		  ...
		  int budget)
{
	...
	for (count = 0; count < budget; count++, tp->cur_rx++) {
		...
		/*
		 * Zero-copy optimization: Single DMA sync only.
		 * Removed dma_sync_single_for_device() as the buffer
		 * ownership is immediately returned to the NIC.
		 */
		...
		/*
		 * Copy packet data directly. For real zero-copy we would
		 * need page-flip buffer management which requires more
		 * significant changes. This optimization focuses on
		 * removing the second DMA sync and using net_prefetch.
		 */
		skb_copy_to_linear_data(skb, rx_buf, pkt_size);
		skb->tail += pkt_size;
		skb->len = pkt_size;

		/* Note: Second dma_sync_single_for_device removed for latency */
...
static irqreturn_t rtl8169_interrupt(int irq, void *dev_instance)
{
	...
	if ((status & 0xffff) == 0xffff || !(status & tp->irq_mask))
		...
		if (status & (TxOK | TxErr)) {
			rtl_tx(tp->dev, tp, 100);
			rtl_ack_events(tp, status & (TxOK | TxErr));
		}
