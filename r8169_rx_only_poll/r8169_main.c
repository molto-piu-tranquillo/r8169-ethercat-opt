...
/*
 * EtherCAT optimized xmit with on-demand TX cleanup
 *
 * Poll에서 TX 처리를 제거했으므로, xmit 시점에 TX 디스크립터 정리
 * - TX 슬롯이 부족해지기 전에 미리 정리
 * - RX 경로의 지연에 영향을 주지 않음
 */
				      ...
				      struct net_device *dev)
{
	...
	/*
	 * EtherCAT 최적화: TX 디스크립터 정리를 xmit에서 수행
	 * Poll에서 제거된 rtl_tx()를 여기서 호출
	 * 임계치: 슬롯이 절반 이하로 떨어지면 정리
	 */
	if (rtl_tx_slots_avail(tp) < NUM_TX_DESC / 2)
		rtl_tx(dev, tp, 64);

	if (unlikely(!rtl_tx_slots_avail(tp))) {
		/* 정리 후에도 슬롯 없으면 한번 더 시도 */
		rtl_tx(dev, tp, 256);
...
/*
 * EtherCAT optimized poll - RX only processing
 *
 * TX/RX 완전 분리:
 * - Poll에서는 RX만 처리 (지연 민감)
 * - TX 정리는 xmit 시점에 필요할 때만 수행 (지연 둔감)
 *
 * 이점:
 * - RX 처리 시간에서 TX 오버헤드 제거
 * - Poll Duration: 15-20μs → 8-10μs 예상
 */
...
{
	tp->irq_mask = RxOK | RxErr | LinkChg;
