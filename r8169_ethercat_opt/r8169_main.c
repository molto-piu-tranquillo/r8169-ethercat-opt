...
/* EtherCAT optimization: busy poll after TX completion */
static unsigned int busy_poll_us;
module_param(busy_poll_us, uint, 0644);
MODULE_PARM_DESC(busy_poll_us, "Busy poll time (us) after TX for EtherCAT (0=disabled, recommended: 30-50)");
...
/*
 * rtl_tx - Process TX completions
 * Returns: number of TX packets completed (0 if none)
 *
 * EtherCAT optimization: Changed return type from void to int
 * to allow poll function to know if TX work was done
 */
...
/*
 * EtherCAT optimization: Check if RX packets are pending
 * Returns true if there are packets waiting to be processed
 */
static inline bool rtl_rx_pending(struct rtl8169_private *tp)
{
	unsigned int entry = tp->cur_rx % NUM_RX_DESC;
	struct RxDesc *desc = tp->RxDescArray + entry;
	u32 status = le32_to_cpu(READ_ONCE(desc->opts1));

	return !(status & DescOwn);
}

/*
 * EtherCAT optimization: Check if TX completions are pending
 * Returns true if there are TX descriptors to be cleaned up
 */
static inline bool rtl_tx_pending(struct rtl8169_private *tp)
{
	return READ_ONCE(tp->cur_tx) != tp->dirty_tx;
}
...
/*
 * EtherCAT-optimized NAPI poll function
 *
 * Optimization strategy for EtherCAT workloads:
 * - EtherCAT pattern: TX send -> ~25μs network RTT -> RX response
 * - Default: TX interrupt -> poll -> RX interrupt -> poll (2 interrupts)
 * - Optimized: TX interrupt -> poll -> busy-wait -> catch RX (1 interrupt)
 *
 * The busy_poll_us parameter controls how long to spin-wait for RX
 * after processing TX. Set to 0 to disable (default behavior).
 */
...
static int rtl8169_poll(struct napi_struct *napi, int budget)
{
	...
	/*
	 * EtherCAT optimization: If TX was completed but no RX yet,
	 * busy-poll briefly to catch the response without another interrupt.
	 * This trades CPU cycles for lower latency.
	 */
	if (busy_poll_us > 0 && tx_done > 0 && work_done == 0) {
		ktime_t deadline = ktime_add_us(ktime_get(), busy_poll_us);

		while (ktime_before(ktime_get(), deadline)) {
			if (rtl_rx_pending(tp)) {
				work_done = rtl_rx(dev, tp, budget);
				break;
			}
			cpu_relax();
