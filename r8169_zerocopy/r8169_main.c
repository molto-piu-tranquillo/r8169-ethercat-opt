...
static const struct pci_device_id rtl8169_pci_tbl[] = {
	...
	{ PCI_VENDOR_ID_DLINK, 0x4300, PCI_VENDOR_ID_DLINK, 0x4b10, 0, 0 },
...
#define RX_CONFIG_ACCEPT_MASK 0x3f
	...
	LinkUp =
		(1
		 << 4), /* Wake up when the cable connection is re-established */
...
/* Zero-Copy RX buffer management structure */
struct rtl_rx_buffer {
	dma_addr_t dma;
	struct page *page;
...
/* Zero-copy RX constants - MUST match R8169_RX_BUF_SIZE (PAGE_SIZE) */
#define RTL_RX_PAGE_SIZE R8169_RX_BUF_SIZE
#define RTL_SKB_PAD (NET_SKB_PAD + NET_IP_ALIGN)
#define RTL_RX_BUF_SIZE_ZC SKB_DATA_ALIGN(RTL_SKB_PAD + RTL_RX_PAGE_SIZE)
#define RTL_RX_DMA_ATTR (DMA_ATTR_SKIP_CPU_SYNC | DMA_ATTR_WEAK_ORDERING)
...
struct rtl8169_private {
	...
	struct rtl_rx_buffer rx_buffer[NUM_RX_DESC]; /* Zero-copy Rx buffers */
...
MODULE_DESCRIPTION(
	"RealTek RTL-8169 Gigabit Ethernet driver - True Zero-Copy RX");
...
static bool rtl_loop_wait_high(struct rtl8169_private *tp,
			       const struct rtl_cond *c, unsigned long d, int n)
{
...
#define DECLARE_RTL_COND(name)                                       \
	static bool name##_check(struct rtl8169_private *);          \
                                                                     \
	static const struct rtl_cond name = { .check = name##_check, \
					      .msg = #name };        \
                                                                     \
...
void r8169_get_led_name(struct rtl8169_private *tp, int idx, char *buf,
			int buf_len)
{
...
{
	RTL_W32(tp, EPHYAR,
		EPHYAR_WRITE_CMD | (value & EPHYAR_DATA_MASK) |
...
static u16 rtl_ephy_read(struct rtl8169_private *tp, int reg_addr)
{
	...
	return rtl_loop_wait_high(tp, &rtl_ephyar_cond, 10, 100) ?
		       RTL_R32(tp, EPHYAR) & EPHYAR_DATA_MASK :
		       ~0;
}
			      ...
			      u32 data)
{
	...
	RTL_W32(tp, OCPAR,
		OCPAR_FLAG | ((u32)mask & 0x0f) << 12 | (reg & 0x0fff));
...
static void r8168ep_ocp_write(struct rtl8169_private *tp, u8 mask, u16 reg,
			      u32 data)
{
	_rtl_eri_write(tp, reg, ((u32)mask & 0x0f) << ERIAR_MASK_SHIFT, data,
		       ERIAR_OOB);
}
...
{
	RTL_W32(tp, EFUSEAR,
		(reg_addr & EFUSEAR_REG_MASK) << EFUSEAR_REG_SHIFT);

	return rtl_loop_wait_high(tp, &rtl_efusear_cond, 100, 300) ?
		       RTL_R32(tp, EFUSEAR) & EFUSEAR_DATA_MASK :
		       ~0;
}
...
static int rtl8169_set_eee(struct net_device *dev, struct ethtool_eee *data)
{
	...
	if (!ret)
		tp->eee_adv =
			phy_read_mmd(dev->phydev, MDIO_MMD_AN, MDIO_AN_EEE_ADV);
	return ret;
}
...
void r8169_apply_firmware(struct rtl8169_private *tp)
{
	...
	if (tp->rtl_fw) {
		...
		phy_read_poll_timeout(tp->phydev, MII_BMCR, val,
				      !(val & BMCR_RESET), 50000, 600000, true);
	}
}
...
static void rtl_rar_exgmac_set(struct rtl8169_private *tp, const u8 *addr)
{
	...
	rtl_eri_write(tp, 0xf0, ERIAR_MASK_1111,
		      get_unaligned_le16(addr) << 16);
...
{
	u32 val = TX_DMA_BURST << TxDMAShift | InterFrameGap
...
static void rtl_csi_write(struct rtl8169_private *tp, int addr, int value)
{
	...
	RTL_W32(tp, CSIAR,
		CSIAR_WRITE_CMD | (addr & CSIAR_ADDR_MASK) | CSIAR_BYTE_ENABLE |
			func << 16);
...
static u32 rtl_csi_read(struct rtl8169_private *tp, int addr)
{
	...
	RTL_W32(tp, CSIAR,
		(addr & CSIAR_ADDR_MASK) | func << 16 | CSIAR_BYTE_ENABLE);
...
static void rtl8168g_set_pause_thresholds(struct rtl8169_private *tp, u8 low,
					  u8 high)
{
...
static void rtl_hw_start_8168c_2(struct rtl8169_private *tp)
{
	static const struct ephy_info e_info_8168c_2[] = {
		{ 0x01, 0, 0x0001 }, { 0x03, 0x0400, 0x0020 }
	};
...
{
	static const u16 fix_data[] = { /* 0xf800 */ 0xe008,
					0xe00a,
					0xe00c,
					0xe00e,
					0xe027,
...
static void rtl_hw_start_8102e_1(struct rtl8169_private *tp)
{
	static const struct ephy_info e_info_8102e_1[] = {
		{ 0x01, 0, 0x6e65 }, { 0x02, 0, 0x091f }, { 0x03, 0, 0xc2f9 },
		{ 0x06, 0, 0xafb5 }, { 0x07, 0, 0x0e00 }, { 0x19, 0, 0xec80 },
		{ 0x01, 0, 0x2e65 }, { 0x01, 0, 0x6e65 }
	};
	u8 cfg1;
...
static void rtl_hw_start_8105e_1(struct rtl8169_private *tp)
{
	static const struct ephy_info e_info_8105e_1[] = {
		{ 0x07, 0, 0x4000 }, { 0x19, 0, 0x0200 }, { 0x19, 0, 0x0020 },
		{ 0x1e, 0, 0x2000 }, { 0x03, 0, 0x0001 }, { 0x19, 0, 0x0100 },
		{ 0x19, 0, 0x0004 }, { 0x0a, 0, 0x0020 }
	};
...
static void rtl_hw_start_8402(struct rtl8169_private *tp)
{
	static const struct ephy_info e_info_8402[] = {
		{ 0x19, 0xffff, 0xff64 }, { 0x1e, 0, 0x4000 }
	};
...
static void rtl_hw_start_8106(struct rtl8169_private *tp)
{
	...
	RTL_W32(tp, MISC,
		(RTL_R32(tp, MISC) | DISABLE_LAN_EN) & ~EARLY_TALLY_EN);
	RTL_W8(tp, MCU, RTL_R8(tp, MCU) | EN_NDP | EN_OOB_RESET);
	RTL_W8(tp, DLLPR, RTL_R8(tp, DLLPR) & ~PFM_EN);
...
static void rtl_hw_start_8125a_2(struct rtl8169_private *tp)
{
	static const struct ephy_info e_info_8125a_2[] = {
		{ 0x04, 0xffff, 0xd000 }, { 0x0a, 0xffff, 0x8653 },
		{ 0x23, 0xffff, 0xab66 }, { 0x20, 0xffff, 0x9455 },
		{ 0x21, 0xffff, 0x99ff }, { 0x29, 0xffff, 0xfe04 },

		{ 0x44, 0xffff, 0xd000 }, { 0x4a, 0xffff, 0x8653 },
		{ 0x63, 0xffff, 0xab66 }, { 0x60, 0xffff, 0x9455 },
		{ 0x61, 0xffff, 0x99ff }, { 0x69, 0xffff, 0xfe04 },
	};
...
static void rtl_hw_start_8125b(struct rtl8169_private *tp)
{
	static const struct ephy_info e_info_8125b[] = {
		{ 0x0b, 0xffff, 0xa908 }, { 0x1e, 0xffff, 0x20eb },
		{ 0x4b, 0xffff, 0xa908 }, { 0x5e, 0xffff, 0x20eb },
		{ 0x22, 0x0030, 0x0020 }, { 0x62, 0x0030, 0x0020 },
	};
...
/* Zero-copy: Allocate and setup a single RX buffer */
static bool rtl_alloc_rx_buffer(struct rtl8169_private *tp, unsigned int entry)
{
	struct rtl_rx_buffer *rx_buf = &tp->rx_buffer[entry];
	...
	dma = dma_map_page(d, page, 0, RTL_RX_PAGE_SIZE, DMA_FROM_DEVICE);
	if (unlikely(dma_mapping_error(d, dma))) {
		__free_page(page);
		return false;
	}
	...
	rx_buf->dma = dma;
	rx_buf->page = page;
	rx_buf->page_offset = RTL_SKB_PAD;
...
static void rtl8169_rx_clear(struct rtl8169_private *tp)
{
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		struct rtl_rx_buffer *rx_buf = &tp->rx_buffer[i];

		if (rx_buf->page) {
			dma_unmap_page(d, rx_buf->dma, RTL_RX_PAGE_SIZE,
				       DMA_FROM_DEVICE);
...
static int rtl8169_init_ring(struct rtl8169_private *tp)
{
	...
	memset(tp->rx_buffer, 0, sizeof(tp->rx_buffer));
		  ...
		  int budget)
{
	...
	for (count = 0; count < budget; count++, tp->cur_rx++) {
		...
		dma_sync_single_for_cpu(d, rx_buf->dma, RTL_RX_PAGE_SIZE,
					DMA_FROM_DEVICE);

		page = rx_buf->page;
		data = page_address(page) + rx_buf->page_offset;

		/* Prefetch the packet data */
		net_prefetch(data);

		/*
		 * Zero-copy path: Use napi_build_skb to build SKB
		 * directly from the DMA buffer.
		 * For small packets, copy is actually faster due to
		 * page allocation overhead.
		 */
		if (pkt_size <= 256) {
			/* Small packet: use copy for efficiency */
			...
			skb->tail += pkt_size;
			skb->len = pkt_size;
			/* Sync back for device - buffer is reused */
			dma_sync_single_for_device(d, rx_buf->dma,
						   RTL_RX_PAGE_SIZE,
						   DMA_FROM_DEVICE);
		} else {
			/* Large packet: zero-copy with page replacement */
			struct page *new_page;
			dma_addr_t new_dma;

			/* Allocate a new page to replace this one */
			new_page = alloc_page(GFP_ATOMIC | __GFP_COMP);
			...
			/* Map the new page for DMA */
			new_dma = dma_map_page(d, new_page, 0, RTL_RX_PAGE_SIZE,
					       DMA_FROM_DEVICE);
			if (unlikely(dma_mapping_error(d, new_dma))) {
			...
			/* Unmap the old page - it's going to SKB */
			dma_unmap_page(d, rx_buf->dma, RTL_RX_PAGE_SIZE,
				       DMA_FROM_DEVICE);

			/* Build SKB from the old page */
			skb = napi_build_skb(page_address(page),
					     RTL_RX_BUF_SIZE_ZC);
			if (unlikely(!skb)) {
				/* Failed to build SKB, restore old buffer */
				dma_unmap_page(d, new_dma, RTL_RX_PAGE_SIZE,
					       DMA_FROM_DEVICE);
				__free_page(new_page);
				/* Re-map the old page */
				rx_buf->dma = dma_map_page(d, page, 0,
							   RTL_RX_PAGE_SIZE,
							   DMA_FROM_DEVICE);
				dev->stats.rx_dropped++;
				goto release_descriptor;
			...
			/* Setup SKB with proper offsets */
			skb_reserve(skb, rx_buf->page_offset);
			__skb_put(skb, pkt_size);

			/* Update the buffer to use the new page */
			rx_buf->page = new_page;
			rx_buf->dma = new_dma;
			rx_buf->page_offset = RTL_SKB_PAD;
			desc->addr = cpu_to_le64(new_dma);
		}
...
static void rtl_task(struct work_struct *work)
{
	...
	if (test_and_clear_bit(RTL_FLAG_TASK_TX_TIMEOUT, tp->wk.flags)) {
		...
		ret = pci_disable_link_state(
			tp->pci_dev, PCIE_LINK_STATE_L1 | PCIE_LINK_STATE_L0S);
		if (!ret)
			netdev_warn_once(tp->dev,
...
static int rtl_open(struct net_device *dev)
{
	...
	retval = request_irq(tp->irq, rtl8169_interrupt, irqflags, dev->name,
			     tp);
	if (retval < 0)
...
static int rtl_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (IS_ERR(tp->clk))
		return dev_err_probe(&pdev->dev, PTR_ERR(tp->clk),
